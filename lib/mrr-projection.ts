...
const MATURE_BUFFER_DAYS = 15;       // a month is "settled" 15d after month end
...
function isMature(snap: MrrDailySnapshot): boolean {
  if (!snap.computed_at) return false;
  ...
  const computed = new Date(snap.computed_at);
  const diffDays = (computed.getTime() - monthEnd.getTime()) / (1000 * 60 * 60 * 24);
