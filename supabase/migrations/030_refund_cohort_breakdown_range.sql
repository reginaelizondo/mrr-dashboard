-- ============================================
-- 030 — Cohort-basis breakdowns by DAY range (fixes weekly/sub-month flooring)
-- ============================================
-- The original refund_cohort_breakdown (migration 021) only accepts month
-- bounds (start_month/end_month), so the cohort segmentation sub-tables
-- (By SKU / country / duration) always floored any selection to whole months
-- and ignored the Weekly/Monthly toggle entirely. A "week of Jul 27" selection
-- silently widened to all of July + August, blending a bad week into a 2-month
-- average (e.g. ht_c read 161/2,978 = 5.4% instead of the week's 28/183 = 15.3%).
--
-- This adds a day-granular sibling with the exact same body but DATE bounds
-- (inclusive on both ends, matching refund_cohorts_weekly in migration 024).
-- The month version is kept for backward compatibility; the app now calls this.

CREATE OR REPLACE FUNCTION refund_cohort_breakdown_range(
...
RETURNS TABLE(
  ...
  WITH bounds AS (
    SELECT start_date AS d0, end_date AS d1
  ),
  live AS (
    SELECT
      CASE
